...
bool rrt::StartRRT2(Robotmodel& model, std::ostream& sout) {
	_model = model;
...
//	cout << "RRT goal" << " " << qgoal *180.0 / M_PI << endl;
	...
	while (iter < 5000000) {
		...
		std::vector<double> node = RandomConfig(); 
		// random config or goal tree�� �� config
...
std::vector<double> rrt::RandomConfig() {
	...
	//cout << "new" << R[0] <<"\t" << R[1] << "\t" << R[2] << "\t" << R[3] << "\t" << R[4] << "\t" << R[5] << "\t" << R[6]  << endl;
	//cout << "no" << endl;
	return R;
}
...
int rrt::Extend(std::vector<double> &node, RRTNode* &near) {
	...
	if (distance < step_size) {
	    if (!checkCollision3(_model, node)) { // False : Collision-free
			RRTNode *old = near;
...
int rrt::ConstrainedExtend(std::vector<double> &node, RRTNode* &near) {
	...
	for (int i = 0; i < goal.size(); i++) {
		...
		//qs_old.push_back(100.0);
	}

	while (true) {
		// random config �� nearest config ������ norm�� step size ���� ���� ��
		if (std_norm(node, qs, DoF_size) < step_size) {
			node = qs;
			return REACHED;
		}
		else if (std_norm(node, qs, DoF_size) - std_norm(qs_old, node, DoF_size) > 0.0) {
			// near���� random config �������� extend�� ��尡 
			node = qs_old;
			return TRAPPED;
		}
		qs_old = qs;

		//if (std_norm(qs, qs_old, DoF_size) < step_size) {
		//	node = qs_old;
		//	return TRAPPED;
		//}

		// near���� node(random config) �������� �̵���
		for (int i = 0; i< goal.size(); i++)
			qs[i] = (qs[i] + min(step_size, std_norm(node, qs, DoF_size)) * (node[i]-qs[i]) / std_norm(node, qs, DoF_size));

		// qs_old : near / qs : extend
		//cout <<"qs" <<"\t"<< qs[0] <<"\t"<< qs[1] << "\t" << qs[2] << "\t" << qs[3] << "\t" << qs[4] << "\t" << qs[5] << "\t" << qs[6] << endl;
		//cout <<"qold" <<"\t"<< qs_old[0] << "\t" << qs_old[1] << "\t" << qs_old[2] << "\t" << qs_old[3] << "\t" << qs_old[4] << "\t" << qs_old[5] << "\t" << qs_old[6] << endl;
...
bool rrt::ConstrainedExtend2(std::vector<double> &path1, std::vector<double> &path2, std::vector<double> &path3)
{
	std::vector<double> q_goal, q_init, qs_old;
	q_goal = path2;
	q_init = path1;
	...
	while (true) {
		if (std_norm(q_goal, q_init, DoF_size) < step_size) {
			path3 = q_init;
			//cout << "true" << endl;
			extend = true;
			break;
		...
		for (int i = 0; i< goal.size(); i++)
			q_init[i] = (q_init[i] + min(step_size, std_norm(q_goal, q_init, DoF_size)) * (q_goal[i] - q_init[i]) / std_norm(q_init, q_goal, DoF_size));

		project_flag = ProjectConfig(_model, q_goal, q_init);
		...
		if (!project_flag) {
			...
			if (!checkCollision3(_model, q_init)) {
...
//	T0_c.topLeftCorner(3,3) = model.refer_rot;
	...
	//		cout << Rot_temp.transpose() << endl;
		...
		//cout <<"qerr"<< q_error.transpose() << endl;
		...
		//	else if (qs[i] < -M_PI)
		//		qs[i] += 2 * M_PI;
		//cout << "qsproj" << "\t" << qs[0] << "\t" << qs[1] << "\t" << qs[2] << "\t" << qs[3] << "\t" << qs[4] << "\t" << qs[5] << "\t" << qs[6] << endl;
		//	cout <<"qold" << qold[0] << qold[1] << qold[2] << qold[3] << qold[4] << qold[5] << qold[6] << endl;

		// Algorithm 4 - line 7 : stuck here
		//std_norm(qs, qold, DoF_size) > 2 * 5 / 180.0 * M_PI || 
//		if (!OutsideJointLimit(qs)) {
//////			cout << std_norm(qs, qold, DoF_size) <<"\t" <<"sub case 3" << endl;
////			//cout << "joint limit!!" << endl;
////
////	//		qold = qs;
//			flag = false;
//			break;
...
bool rrt::SmoothCPath(std::ostream& sout, std::istream& sinput)
{
	std::string fullinput;
...
bool rrt::ShortcutCSmoothing() {
	if (path.size() <= 2) return false;
	int i = ((double)rand() / RAND_MAX)*path.size();
	int j = ((double)rand() / RAND_MAX)*path.size();
	while (abs(j - i)<2) { // same number or adjacent number
		i = ((double)rand() / RAND_MAX)*path.size();
		j = ((double)rand() / RAND_MAX)*path.size();
	}
	if (j < i) { //make sure i is the smaller number
	...
	// path : std::vector< std::vector<double> > 
	//if (CheckTraj(path[i], path[j])) { // i->j ���� step length��ŭ �̵����� �� collision ������ i+1~j���� ��� ����
	//	path.erase(path.begin() + i + 1, path.begin() + j);
	//	return true;
	//}
	std::vector<double> path_new;
//	cout << i << "\t" << j << endl;
//	cout << "start" << endl;
	if (ConstrainedExtend2(path[i], path[j], path_new)) {
	//	cout << "4" << endl;
	//	cout << i << "\t" << j << endl;
		// �ܼ��� ����� �ȵ���. �׷� �Ǵ� q�鳢�� ���� �� ó�� ��. 
		path.erase(path.begin() + i + 1, path.begin() + j);
	//	cout << "erase" << endl;
		//	cout << "1" << endl;
		path.insert(path.begin() + i + 1, path_new);
		//cout << "2" << endl;
	//	cout << "insert" << endl;
		return true;
	}
...
bool rrt::CheckTraj(std::vector<double> &a, std::vector<double> &b) {
	...
	while (p != b) {
		if (checkCollision3(_model, p)) { return false;
		break;
		}
...
bool rrt::checkCollision3(Robotmodel model, std::vector<double> &config) {
	model.q.resize(DoF_size);
	for (int i = 0; i < DoF_size; i++)
	...
	if (!left) {
		...
		// Box1[1].vAxis[0] = Rot_temp2.col(0);
		// Box1[1].vAxis[1] = Rot_temp2.col(1);
		// Box1[1].vAxis[2] = Rot_temp2.col(2);
		// Box1[1].vPos = CalcBodyToBaseCoordinates(*model.model, model.q, model.body_id[4], model.com_id[4], true);

		// Matrix3d Rot_temp3 = model.Rot*Rot_arm_link2(model.q); // link 2
		// Box1[2].vAxis[0] = Rot_temp3.col(0);
		// Box1[2].vAxis[1] = Rot_temp3.col(1);
		// Box1[2].vAxis[2] = Rot_temp3.col(2);
		// Box1[2].vPos = CalcBodyToBaseCoordinates(*model.model, model.q, model.body_id[1], model.com_id[1], true);
		...
		// Box2[1].vAxis[2] = Vector3d(0, 0, 1);
		// Box2[1].vPos = Vector3d(0.2212, 0.3192, 1.0135);
...
bool rrt::OutsideJointLimit(std::vector<double> q) {
	for (int i = 0;i < q.size();i++) {
		if (q[i] > upper_limit(i)) {
			cout << q[i] << "\t" << upper_limit(i) << endl;
			return false;
		}
		else if (q[i] < lower_limit(i)) {
			cout << q[i] << "\t" << lower_limit(i) << endl;
